...
export default __component("Button", (props) => {
    ...
    const __view = (props) => {
        const marker = props.loading
            ? _jsx(Icon, { name: "loader", class: "animate-spin" })
        ...
        const body = props.trailing ? _jsxs(_Fragment, { children: [content, marker, tail] })
            : _jsxs(_Fragment, { children: [marker, content, tail] });
        if (props.to || props.href) {
            return _jsx("a", { ...attrs, "aria-busy": __live(() => props.loading || undefined), "aria-disabled": __live(() => props.disabled || props.loading || undefined), href: __live(() => props.to || props.href), class: __live(() => [__styles.root, props.className ?? props.class]), children: body });
        }
